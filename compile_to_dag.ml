...
let ptg_of_typed_ast types c =
    let one_step ptg_circ tp = match ptg_circ with
        | Id x   ->
                Dags.identity x 
        | IdPoly -> 
                let (n,m) = extract_types tp types in   
...
let tests = List.map typecheck_and_compile [ test1a; test1b; test1c; test2; test3; test4; test5; test6 ];;
